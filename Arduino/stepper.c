...
void st_wake_up()
{
  ...
  // Sleep Pin config for stepper drivers.
  #ifdef DEFAULTS_RAMPS_BOARD
    
      STEPPER_SLEEP_PORT(0) |= (1 << STEPPER_SLEEP_BIT(0));
      STEPPER_SLEEP_PORT(1) |= (1 << STEPPER_SLEEP_BIT(1));
      STEPPER_SLEEP_PORT(2) |= (1 << STEPPER_SLEEP_BIT(2));
    
  #endif // Ramps Board

   // Reset Pin config for stepper drivers.
  #ifdef DEFAULTS_RAMPS_BOARD
      STEPPER_RESET_PORT(0) |= (1 << STEPPER_RESET_BIT(0));
      STEPPER_RESET_PORT(1) |= (1 << STEPPER_RESET_BIT(1));
      STEPPER_RESET_PORT(2) |= (1 << STEPPER_RESET_BIT(2));
...
void stepper_init()
{
  // Configure step and direction interface pins
    ...
    STEPPER_ENABLE_DDR(0) |= 1<<STEPPER_ENABLE_BIT(0);
    STEPPER_ENABLE_DDR(1) |= 1<<STEPPER_ENABLE_BIT(1);
    STEPPER_ENABLE_DDR(2) |= 1<<STEPPER_ENABLE_BIT(2);

    STEPPER_SLEEP_DDR(0) |= 1<<STEPPER_SLEEP_BIT(0);
    STEPPER_SLEEP_DDR(1) |= 1<<STEPPER_SLEEP_BIT(1);
    STEPPER_SLEEP_DDR(2) |= 1<<STEPPER_SLEEP_BIT(2);

    STEPPER_RESET_DDR(0) |= 1<<STEPPER_RESET_BIT(0);
    STEPPER_RESET_DDR(1) |= 1<<STEPPER_RESET_BIT(1);
    STEPPER_RESET_DDR(2) |= 1<<STEPPER_RESET_BIT(2);
